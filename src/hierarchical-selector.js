    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        if (!scope.syncData && scope.isAsync) {
          scope.data = [];
          var items = scope.loadChildItems({
            parent: null
          });
      ...
      controller: function($scope, $document, $window, $interpolate) {
        ...
        $scope.deselectItem = function(item, $event) {
          ...
          if ($scope.onSelectionChanged) {
            $scope.onSelectionChanged({
              items: $scope.selectedItems.length ? $scope.selectedItems : undefined
            });
          }
        };
        ...
        $scope.itemSelected = function(item) {
          if (($scope.useCanSelectItemCallback && $scope.canSelectItem({
              item: item
            }) === false) || ($scope.selectOnlyLeafs && selectorUtils.hasChildren(item, $scope.isAsync))) {
            return;
          }
          var itemMeta = selectorUtils.getMetaData(item);
